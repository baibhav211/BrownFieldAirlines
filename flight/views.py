    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            try:
                ...
                ticket.seat_no = generate_random_seat()
                ticket.save()
                if t2:
                    ticket2 = Ticket.objects.get(id=ticket2_id)
                    ticket2.status = 'CONFIRMED'
                    ticket2.seat_no = generate_random_seat()
                    ticket2.save()
...
def seatselection(request):
    return render(request, 'flight/seats.html')


def generate_random_seat():
    alphabets = 'ABCDEF'
    digits = [str(i).zfill(2) for i in range(1, 31)]
    seat_number = random.choice(digits) + random.choice(alphabets)
    seat_number2 = seat_number.replace("0", "")
    return seat_number2
...
def checkin(request):
    if request.method == 'POST':
        ref_no = request.POST.get('ticket_reference', None)
        last_name = request.POST['last_name']
        try:
            ticket = Ticket.objects.get(ref_no=ref_no)
            if ticket.user.last_name != last_name:
                return render(request, 'flight/checkin_search.html', {'warning_lname': "Last Name does not match"})
            passengers = ticket.passengers.all()
            checked_in = ticket.checked_in
            ticket_status = ticket.status
            seat_class = ticket.seat_class
            seat_class1 = seat_class.capitalize()
            if(checked_in == False and ticket_status=='CONFIRMED'):
                return render(request, 'flight/checkin.html',{'ticket': ticket, 'passengers':passengers,'ref_no':ref_no,
                'seat_class1':seat_class1,'page': 'checkin' })
            else:
                return render(request,'flight/checkin_status.html',{'ticket': ticket, 'passengers':passengers,'ref_no':ref_no,'ticket_status':ticket_status ,'page': 'checkin'})
        except Ticket.DoesNotExist:
            return render(request, 'flight/checkin_search.html', {'ticket_reference': ref_no, 'warning_ref_no': 'The Ticket Reference Number you requested is not valid. Enter valid Ticket Reference Number.','page': 'checkin'})
    return render(request, 'flight/checkin_search.html', {'page': 'checkin'})
...
def checkin_confirmation(request):
    if request.method == 'POST':
        ref_no = request.POST.get('ticket_reference', False)
        ticket = get_object_or_404(Ticket, ref_no=ref_no)
        checked_in = ticket.checked_in
        if (checked_in == False):
            ticket.checked_in = True
            ticket.save()
    return render(request, 'flight/checkin_confirmation.html', {'ref_no': ref_no, 'page': 'checkin'})
    # passenger = get_object_or_404(Passenger, pk=passenger_id)
    # passenger.seat = request.POST['seat_no']
    # passenger.save()
